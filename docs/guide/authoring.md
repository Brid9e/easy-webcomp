...
空间页里每张卡片显示的不是实时组件，而是一张静态截图 `docs/public/snapshots/<空间>/<组件名>.png` —— 空间里组件一多，页面上同时挂十几个运行时又慢又没必要。点击卡片进详情页、或点右上角全屏，挂的才是真实元素。

截图由 `pnpm run snapshot` 生成，**要先跑过 `pnpm run build`**（它引的正是 `dist/cdn/<空间>/index.js`）。改了组件样式或结构之后要重跑一次，否则卡片还是旧样子 —— 卡片自己不会发现源码变了。图片提交进 git，所以生成是确定性的：脚本把 `Date.now()` 钉在固定时刻，`my-list` 那种现算的时间列不会每次跑都产生无意义的二进制 diff。
