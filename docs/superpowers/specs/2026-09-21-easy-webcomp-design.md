...
**为什么单包**：`tokens` / `shared` / `runtime` 必须与组件**版本严格同步**（token 改了组件就得跟着发），拆包只会带来版本对齐成本。宿主若想单独使用 token，通过 exports 子路径提供 `easy-webcomp/tokens.css` 即可，无需真的拆包。
...
**命名约定**：tag 前缀统一 `ew-`；attribute 使用 kebab-case；事件名由 `meta.events` 声明，最终映射为 `ew-<name>` 形式的 DOM CustomEvent。
...
- Vue 适配器：`createApp` + `shallowRef` 装 props + render 函数内调 `getComponent()`；事件经 `provide(EW_EMIT_KEY, emit)` 注入，组件侧用 `useEmit()` 取出。
...
机制：运行时把已加载的库挂到 `window.__EW_SHARED__[name]`。任何 bundle 加载时**先查全局注册表** —— 命中直接复用，未命中才加载并登记。
...
`src/tokens/` 产出 `tokens.css`（`:root { --ew-color-primary: ... }`）与 `tokens.ts`。

选 CSS 自定义属性做主题机制，是因为**它能穿透 Shadow DOM 边界**（靠继承而非选择器匹配）。宿主换肤只需覆盖 `--ew-*`，无需逐个 `::part()` 暴露。这使 shadow 方案能同时满足「强隔离」与「可定制」。
...
1. **就近**：`<ew-provider>` 包裹 —— 作用域隔离，同页多实例可用不同上下文
2. **全局**：`window.__EW_HOST__ = { request, getToken, locale, theme }`
3. **兜底**：组件自身属性
...
| self-contained | 全量 | `dist/index.js` / `dist/ew-all.iife.js` | 全组件 |
| shared | 单组件 ESM | `dist/esm-shared/<name>.js` | 运行时 external，只 export |
| shared | 单组件 IIFE | `dist/cdn-shared/<name>.js` | 运行时 external，引入即注册 |
| shared | 全量 | `dist/esm-shared/index.js` / `dist/cdn-shared/ew-all.iife.js` | 全组件 |
...
```
easy-webcomp/button          → export class（无副作用）
easy-webcomp/button/define   → 引入即注册（有副作用）
```
...
└── 每个组件页
    ...
    └── [WC 模式]   import 'virtual:ew-wc/<name>'
```

**源码模式**：组件源码直接挂载到页面，获得原生 HMR 与框架 devtools。

**WC 模式**：通过一个 Vite 插件提供 `virtual:ew-wc/<name>` 虚拟模块，内部：import 组件源码 → `defineElement(meta, impl)` 定义 CE → 页面插入 `<ew-biz-chart>`。
...
3. **产物冒烟测试**（Playwright）—— 唯一能验证「打包后」的手段。加载静态页 `<script src="dist/cdn/xxx.js">`，断言：CE 已升级、shadow root 存在、事件能冒泡到 window、**token 换肤生效**（改 `--ew-*` 后断言计算样式变化，直接防住第 8.3 节的 `:host` 坑）。
...
- **二期（依赖共用，本期不做）**：`window.__EW_SHARED__` 注册表、宿主注入协议三级兜底、shared 模式产物、token 换肤能力。
